...
export destroy, LineStyle, add_color_stop, circle, curve, curve_between, draw, get_text_info,
       line, line_to, linear_pattern, move_to, notitles, over, plotpath, qsave, rect,
       set_linestyle, source, stroke, text, text_with_superscript, titles
...
"""
function Cairo.text(ctx::CairoContext, p::Point, fsize, fcolor, txt; horizontal = "left",
                    vertical = "baseline", fname = "Sans")
...
"""
function text_with_superscript(ad, pos, fsize, fcolor, basestr, expstr; basefname = "cmr10",
                               supfname = "cmr7", vertical = "baseline", horizontal = "left")
  sh = -0.37
...
function text_with_superscript(dw::Drawable, args...; kw...)
  text_with_superscript(dw.ctx, args...; kw...)
end
...
"""
add_color_stop(pat::CairoPattern, offset, c::RGBColor) = Cairo.pattern_add_color_stop_rgb(pat,
                                                                                          offset,
                                                                                          c.r,
                                                                                          c.g,
                                                                                          c.b)
function add_color_stop(pat::CairoPattern, offset, c::RGBAColor)
  Cairo.pattern_add_color_stop_rgba(pat, offset, c.r, c.g, c.b, c.a)
end
...
"""
Cairo.rectangle(ctx::CairoContext, p::Point, wh::Point) = Cairo.rectangle(ctx, p.x, p.y, wh.x,
                                                                          wh.y)
...
function Cairo.arc_negative(ctx::CairoContext, p::Point, r, t1, t2)
  Cairo.arc_negative(ctx, p.x, p.y, r, t1, t2)
end
...
"""
Cairo.curve_to(ctx::CairoContext, p1::Point, p2::Point, p3::Point) = Cairo.curve_to(ctx, p1.x,
                                                                                    p1.y, p2.x,
                                                                                    p2.y, p3.x,
                                                                                    p3.y)
function Cairo.curve_to(dw::Drawable, p1::Point, p2::Point, p3::Point)
  Cairo.curve_to(dw.ctx, p1, p2, p3)
end
...
function textx(dw::Drawable, p::Point, fsize, color, fname, txt)
  textx(dw.ctx, p, fsize, color, fname, txt)
end
...
function rect(ctx::CairoContext, b::Box; linestyle = nothing, fillcolor = nothing)
  rect(ctx, Point(b.xmin, b.ymin), Point(b.xmax - b.xmin, b.ymax - b.ymin); linestyle,
       fillcolor)
end
...
"""
function curve(ctx::CairoContext, p0, p1, p2, p3; closed = false, linestyle = nothing,
               fillcolor = nothing)
...
"""
function line(ctx::CairoContext, p::Array{Point}; closed = false, linestyle = nothing,
...
function addtitle(ad, fname)
  ...
  rect(ad2.ctx, Point(0, 0), Point(ad2.width, ad2.height);
       fillcolor = Color(0.9, 1.0, 1.0, 0.7))
