...
function makevector(T::DataType)
    ...
    hname = Symbol(lowercase(string(tname)) * "_hadamard")
    hdname = Symbol(lowercase(string(tname)) * "_hadamarddiv")
    ex = (quote
          # suppose
          #   struct Color
          #     r
          ...
          # this means *(a, p::Color) = Color(a*p.r, a*p.g, a*p.b)
              ...
              # we cannot define our own functions here. (eg hadamard)
              # Because if two modules call this macro, neither will own the function
              # and so one will overwrite the other
              # instead, we give such functions a unique name
              $hname(p::$tname, q::$tname) =  $(Expr(:call, tname, [:(p.$fld * q.$fld) for fld in fnames]...))
              $hdname(p::$tname, q::$tname) =  $(Expr(:call, tname, [:(p.$fld / q.$fld) for fld in fnames]...))
